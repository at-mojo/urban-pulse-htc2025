...
export default function Dashboard() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: We want to fetch reports on mount
  useEffect(() => {
    fetchReports();
  }, []);
  ...
  return (
    <main className="flex flex-1 flex-col items-center justify-center relative pt-[100px]">
            ...
            <div className="w-full h-full flex flex-row gap-4 bg-background rounded-xl p-4 border-2 border-border mt-4">
              ...
              <div className="flex flex-col gap-4 flex-1 h-full">
                <div className="w-full bg-background rounded-xl p-4 border-2 border-border flex flex-row items-center gap-4">
                  <ChevronsUpIcon className="w-32 h-32" color="#e44d4d" />{" "}
                  <span className="text-2xl font-bold font-departure-mono">
                    {
                      reports.filter((report) => report.urgency === "HIGH")
                        .length
                    }{" "}
                ...
                <div className="w-full bg-background rounded-xl p-4 border-2 border-border flex flex-row items-center gap-4">
                  <ChevronUpIcon className="w-32 h-32" color="#f18227" />{" "}
                  <span className="text-2xl font-bold font-departure-mono">
                    {
                      reports.filter((report) => report.urgency === "MEDIUM")
                        .length
                    }{" "}
                ...
                <div className="w-full bg-background rounded-xl p-4 border-2 border-border flex flex-row items-center gap-4">
                  <MinusIcon className="w-32 h-32" color="#0ea55f" />{" "}
                  <span className="text-2xl font-bold font-departure-mono">
                    {
                      reports.filter((report) => report.urgency === "LOW")
                        .length
                    }{" "}
      ...
      {value === "my-reports" && (
        <div className="w-full h-full relative flex flex-1">
          <div className="w-full h-full bg-background rounded-md p-4">
            <h1 className="text-3xl font-bold font-departure-mono">
              My Reports
            </h1>
            <div className="w-full h-full flex flex-row gap-4 bg-background rounded-xl p-4 border-2 border-border mt-4">
              <div className="flex-2 h-full">
                  ...
                  <span className="text-2xl font-bold font-departure-mono">
                    {
                      reports.filter(
                        (report) =>
                          report.urgency === "LOW" && report.userId === user?.id
                      ).length
                    }{" "}
                    Low Priority Reports
                  </span>
      ...
      <ReporterUI fetchReports={fetchReports} />
    </main>
  );
}
